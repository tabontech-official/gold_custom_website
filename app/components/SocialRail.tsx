/**
 * The official brand marks, verbatim from Simple Icons (CC0) — not traced
 * approximations.
 *
 * The set these replace was drawn by hand, and a logo that is *nearly* right is
 * the thing that reads as cheap: Instagram was a stroked rounded-rect with a
 * ring and a dot, which is a sketch OF the mark rather than the mark. These are
 * the paths the brands publish, so they are correct at any size and stay
 * correct without anyone re-eyeballing them.
 *
 * All four are a single filled path on the same 24x24 grid, which is the other
 * half of the fix: the old set mixed ONE stroked outline (Instagram) with three
 * solid glyphs, so the row read as mismatched no matter how it was sized.
 *
 * ponytail: inline paths, no icon package. Four static strings that change only
 * when a brand rebrands do not justify a dependency — and a general-purpose UI
 * icon set is the wrong source anyway, since it ships stylised interpretations
 * of these logos rather than the real marks.
 */
...
/**
 * The store's profiles, in the order they render. `brand` is both the icon key
 * and the `data-social` value app.css hangs the per-brand colour off, so adding
 * a network is one entry here plus one rule there — there is no third place to
 * forget.
 *
 * Lives here rather than in Footer because the footer no longer renders a
 * social row; the rail and the mobile bar below are the only consumers.
 */
const SOCIAL_LINKS = [
  {
    brand: 'instagram',
    label: 'Instagram',
    handle: '@goldcustom_la',
    href: 'https://www.instagram.com/goldcustom_la',
  },
  {
    brand: 'facebook',
    label: 'Facebook',
    handle: 'Gold Custom LA',
  ...
  brand: keyof typeof SOCIAL_ICON_PATHS;
  label: string;
  /** Shown by the left rail when a pill expands; the footer row uses icons only. */
  handle: string;
  href: string;
}>;

/**
 * The link around each of these carries the `aria-label`, so the glyph itself
 * is decorative — `aria-hidden` here, or a screen reader announces the brand
 * twice. `focusable="false"` is for IE/legacy Edge, which put SVGs in the tab
 * order otherwise; it costs nothing and the footer is a long tab-stop already.
 */
function SocialIcon({brand}: {brand: keyof typeof SOCIAL_ICON_PATHS}) {
  return (
    <svg
      width="16"
      height="16"
      ...
      focusable="false"
    >
      <path d={SOCIAL_ICON_PATHS[brand]} />
    </svg>
  );
}
