                ...
                <div className="footer-social">
                  {SOCIAL_LINKS.map(({brand, label, href}) => (
                    <a
                      ...
                      rel="noreferrer"
                      aria-label={label}
                      /* app.css hangs each brand's own hover colour off this. */
                      data-social={brand}
                    >
 ...
 * and the `data-social` value app.css hangs the per-brand hover colour off, so
 * adding a network is one entry here plus one rule there — there is no third
 * place to forget.
 */
 ...
 */
export function SocialIcon({brand}: {brand: keyof typeof SOCIAL_ICON_PATHS}) {
  return (
    <svg
      width="16"
      height="16"
